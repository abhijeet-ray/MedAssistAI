...
def safe_generate_embedding(text: str) -> Optional[List[float]]:
    """
    Generate embedding with retry logic and exponential backoff.
    
    Implements robust retry mechanism to handle Bedrock throttling:
    - Max 6 retries
    - Exponential backoff: 1s, 2s, 4s, 8s, 16s, 32s
    - Jitter to prevent thundering herd
    - Small delay before request to prevent burst
    
    Args:
        text: Text to generate embedding for
    
    Returns:
        Embedding vector as list of floats, or None if all retries fail
    """
    max_retries = 6
    ...
    for attempt in range(max_retries):
        try:
            ...
            if not embedding:
                print(f"Warning: No embedding returned from Titan model")
                return None
            
            # Success - log and return
            if attempt > 0:
                print(f"Embedding generated successfully after {attempt} retries")
            ...
            if 'ThrottlingException' in str(e) or error_code == 'ThrottlingException':
                if attempt < max_retries - 1:
                    ...
                    print(f"Bedrock throttled (attempt {attempt + 1}/{max_retries}), retrying in {delay:.2f}s")
                    time.sleep(delay)
                else:
                    print(f"Bedrock throttled after {max_retries} retries, giving up")
                    return None
            else:
...
def generate_question_embedding(question: str) -> Optional[List[float]]:
    """
    ...
    """
    return safe_generate_embedding(question)
...
def handler(event, context):
    ...
    try:
        ...
        # Check if embedding generation failed due to throttling
        if question_embedding is None:
            print("Embedding generation failed after retries, returning throttle message")
            return {
                'statusCode': 200,
                'headers': {
                    'Access-Control-Allow-Origin': '*',
                ...
                'body': json.dumps({
                    'response': 'Our AI service is experiencing high demand. Please wait a moment and try your question again.',
                    'sources': [],
                    'timestamp': datetime.utcnow().isoformat(),
                    'throttled': True
                })
